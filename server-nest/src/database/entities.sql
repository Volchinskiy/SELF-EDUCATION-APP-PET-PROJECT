...
INSERT INTO question (person_id, title, text, topic) VALUES (1, 'Что такое `Индификатор`?', 'Индификатор это Имена, использующиеся для переменных, функций, меток и других определяемых пользователем объектов. По сути любая переменная это Индификатор она индифицирует значение в памяти.', 'FUNDAMENTAL');
...
INSERT INTO question (person_id, title, text, topic) VALUES (1, 'Что такое `Инструкция`?', 'Инстру́кция или оператор — наименьшая автономная часть языка программирования; команда или набор команд. Программа обычно представляет собой последовательность инструкций. Многие языки различают инструкцию и определение. Различие в том, что инструкция исполняет код, а определение создаёт идентификатор.', 'FUNDAMENTAL');
...
INSERT INTO question (person_id, title, text, topic) VALUES (1, 'Что такое `Выражение`?', 'Выражение в информатике — комбинация значений, констант, переменных, операций и функций, которая может быть интерпретирована в соответствии с правилами конкретного языка.', 'FUNDAMENTAL');
INSERT INTO question (person_id, title, text, topic) VALUES (1, 'Что такое `Оператор`?', 'Оператор или Инстракция - наименшая синтаксическая часть языка программирования исполняемая интерпретатором, средой или компилируемой в машинный код.', 'FUNDAMENTAL');
