...
export default function ButtonItemTheme({title, index}: props) { 
  ...
  const onSelectTheme = () => {
    dispatch(selectTheme(index))    
  }

  return (
    // <button className="content__left-side-button-item --buttonSelected">
  <button onClick={onSelectTheme}  className={`content__left-side-button-item ${ThemeReducer.SelectedTheme === index ? "--buttonSelected" : null}`}>
    <div>
      {
        ThemeReducer.SelectedTheme === index 
        ?
