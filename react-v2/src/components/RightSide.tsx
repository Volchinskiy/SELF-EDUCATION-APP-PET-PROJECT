  ...
  if (selectedQuestion === null) {
    return (
      <div className={`content__right-side  ${uiReducer.showLeftSide ? '--right-side-not-stretched' : ''}`}>
        <div className="content__right-side-body">при выбранном вопросе === null выводить другой ui</div>
      </div>
    );
  }
    ...
    <div className={`content__right-side ${uiReducer.showLeftSide ? '--right-side-not-stretched' : ''}`}>
      <div className="content__right-side-body">
        ...
        <div className="content__right-side-button-wrapper">
          <button onClick={onShowAnswer} className="content__right-side-button">
            Показать Ответ
          </button>
          <button onClick={onNextQuestion} className="content__right-side-button">
            Следующий Вопрос
          </button>
        </div>
      </div>
