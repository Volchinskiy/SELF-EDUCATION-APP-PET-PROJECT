...
export default function ButtonItemTheme({title, index}: props) { 
  ...
  return (
    <button onClick={onSelectTheme}  className={`content__left-side-button-item ${ThemeReducer.SelectedTheme[0] === index ? "--buttonSelected" : null}`}>
      <div>
        {
          ThemeReducer.SelectedTheme[0] === index 
          ?
